/**
 * AI Hedge Fund — Profile Trader v3
 *
 * Single wallet profiler using Method A (closedPositions-based) PnL.
 * More accurate than v2 (cash-flow): correctly captures unredeemed losers
 * via openPos.cashPnl and avoids pre-window redeem inflation.
 * Core profileTrader() is exported for use by bulk-profile-ahf.ts.
 *
 * Usage:
 *   npx tsx scripts/ai-hedge-fund/profile-trader-v3.ts <wallet> [conviction_multiplier]
 *
 * Examples:
 *   npx tsx scripts/ai-hedge-fund/profile-trader-v3.ts 0x23796015f5159c76921dc869b7f95a7c57e2bf16
 *   npx tsx scripts/ai-hedge-fund/profile-trader-v3.ts 0x23796015f5159c76921dc869b7f95a7c57e2bf16 20
 */
...
async function main() {
  ...
  if (!mongoUri) {
    console.warn('⚠ MONGODB_URI not set — will run profiler but skip DB save');
  }
