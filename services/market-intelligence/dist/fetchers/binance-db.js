...
exports.getLatestBinanceFunding = getLatestBinanceFunding;
exports.getLatestBinanceDerivatives = getLatestBinanceDerivatives;
 ...
 */
async function getLatestBinanceFunding(symbol) {
    try {
        const db = db_1.mongoose.connection.db;
        if (!db)
            return null;
        const record = await db.collection('binance_funding_8h').findOne({ symbol: symbol.toUpperCase() }, { sort: { timestamp: -1 } });
        if (!record)
            return null;
        return {
        ...
        logger_1.logger.debug('BinanceDB', `${symbol} funding lookup failed: ${err.message}`);
        return null;
    }
}
/**
 ...
 * Returns null if the collection is empty or symbol not found.
 */
async function getLatestBinanceDerivatives(symbol) {
    try {
        const db = db_1.mongoose.connection.db;
        if (!db)
            return null;
        ...
        logger_1.logger.debug('BinanceDB', `${symbol} derivatives lookup failed: ${err.message}`);
        return null;
    }
}
//# sourceMappingURL=binance-db.js.map