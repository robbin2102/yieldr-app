...
function pct(val, decimals = 4) {
    if (val == null)
        return 'N/A';
    return (val >= 0 ? '+' : '') + val.toFixed(decimals) + '%';
}
// ─── Rich console output ───────────────────────────────────────────────────────
function printCoinSnapshot(docId, snapshot) {
    const sym = snapshot.symbol;
    const ts = snapshot.timestamp.toISOString();
    const ind = snapshot.indicators;
    const price = snapshot.price;
    const der = snapshot.derivatives;
    const comp = snapshot.computed;
    const ms = comp.market_structure;
    const pp = ind.pivot_points;
    const fib = ind.fibonacci;
    const ich = ind.ichimoku;
    const activePatterns = (snapshot.candlestick_patterns ?? []).filter(p => p.value !== 0);
    ...
    for (const cross of comp.ma_crossovers) {
