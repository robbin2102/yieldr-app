...
export async function refreshTrackedCoins(): Promise<{ all: string[]; full: string[]; lite: string[] }> {
  ...
  if (cgCoins.length > 0) {
    ...
    all = intersected.slice(0, config.totalTrackedCoins).map(item => item.symbol.toUpperCase());
  } else {
    logger.warn('Tracker', 'CoinGlass markets unavailable — using hardcoded fallback coin list');
    all = FALLBACK_COIN_PRIORITY
      .filter(sym => taapiSymbols.has(sym) && !EXCLUDE_SYMBOLS.has(sym))
      .slice(0, config.totalTrackedCoins);
    logger.info('Tracker', `Fallback list: ${all.length} coins matched TAAPI symbols`);
  }

  const full = all.slice(0, config.fullDerivativesTier);
  const lite = all.slice(config.fullDerivativesTier);

  await (TrackedCoins as any).findOneAndUpdate({}, {
    $set: {
      updated_at:             new Date(),
      all,
...
export async function loadTrackedCoins(): Promise<{ all: string[]; full: string[]; lite: string[] }> {
  const stored = await (TrackedCoins as any).findOne().sort({ updated_at: -1 });
...
async function fetchCoinGlassMarkets(): Promise<Array<{ symbol: string; openInterest: number | null; price: number | null }>> {
  try {
