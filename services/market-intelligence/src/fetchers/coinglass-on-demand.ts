...
export async function fetchOnDemand(symbol: string): Promise<Record<string, unknown> | null> {
  ...
  logger.info('OnDemand', `${upperSym}: cache miss — fetching 10 per-coin CoinGlass endpoints`);
  ...
  logger.info('OnDemand', `${upperSym}: snapshot upgraded to on_demand tier, cached until ${expiresAt.toISOString()}`);
...
function buildEnrichedDerivatives(
  ...
  if (perCoin.cvd_history.length > 0) {
    const values = perCoin.cvd_history.map((d: any) => d.cvd ?? d.value ?? 0);
    enriched.cvd = {
