...
function buildDerivatives(
  ...
  if (perCoin?.oi_history && perCoin.oi_history.length >= 2) {
    ...
    if (curr && prev1) oiChange1h = ((curr - prev1) / prev1) * 100;
    if (curr && prev4) oiChange4h = ((curr - prev4) / prev4) * 100;
  }

  // CVD from history
  let cvd: { value: number | null; change_1h: number | null; change_4h: number | null } = {
    value: null, change_1h: null, change_4h: null,
  };
  if (perCoin?.cvd_history && perCoin.cvd_history.length > 0) {
    const vals = perCoin.cvd_history.map((d: any) => d.cvd ?? d.value ?? 0);
    cvd = {
  ...
  let oiWeighted: number | null = null;
  let volWeighted: number | null = null;
  ...
  let liqH1 = { long_usd: null as number | null, short_usd: null as number | null, count: null as number | null };
  ...
  if (perCoin?.liq_history && perCoin.liq_history.length > 0) {
    ...
    if (perCoin.liq_history.length >= 4) {
      const h4Longs = perCoin.liq_history.slice(-4).reduce((s: number, d: any) => s + (d?.longLiquidationUsd ?? 0), 0);
      const h4Shorts = perCoin.liq_history.slice(-4).reduce((s: number, d: any) => s + (d?.shortLiquidationUsd ?? 0), 0);
...
async function updateLiquidationLevels(
  ...
  try {
    const buckets = await bucketLiquidations(symbol, liqHistory, currentPrice);
