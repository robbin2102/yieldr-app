...
class Confirmer {
    ...
    async connect() {
        return new Promise((resolve, reject) => {
            ...
            console.log(`[Confirmer] URL: ${config_1.config.wssUser}`);
            ...
            eventBus_1.eventBus.on('trade:submitted', (data) => {
    ...
    async checkPendingOrders() {
        ...
        for (const [orderId, pending] of this.pendingOrders) {
            try {
                ...
                const response = await fetch(`${config_1.config.clobApiBase}/order/${orderId}`, {
                    headers: {
                        'POLY-API-KEY': config_1.config.apiKey,
                        'POLY-SIGNATURE': this.generateSignature(orderId),
                        'POLY-TIMESTAMP': Math.floor(Date.now() / 1000).toString(),
                        'POLY-PASSPHRASE': config_1.config.passphrase,
                    },
                });
                if (!response.ok) {
    ...
    generateSignature(orderId) {
        ...
        return crypto_1.default
            .createHmac('sha256', config_1.config.apiSecret)
            .update(message)
            .digest('base64');
    }
    /**
     ...
     */
    async processFill(pending, fillData) {
        ...
        // Update trade record
        const tradeRecord = await PolyAgentTrade_1.PolyAgentTrade.findById(pending.tradeId);
        if (!tradeRecord) {
        ...
        console.log(`═══════════════════════════════════════════════════════════\n`);
        eventBus_1.eventBus.emit('trade:filled', { tradeId: pending.tradeId });
    }
    /**
    ...
    async handleOrderUpdate(msg) {
        if (msg.type === 'CANCELLATION') {
            ...
            if (pending) {
                ...
                eventBus_1.eventBus.emit('trade:failed', { tradeId: pending.tradeId, error: 'FOK order not filled' });
     ...
     */
    async updateSlippageBuffer(expectedCost, actualCost, slippageUsdc) {
        // Atomic update
        const result = await PolyAgentSlippage_1.PolyAgentSlippage.findByIdAndUpdate('current', {
            $inc: {
