...
const FundingRate1hSchema = new Schema({
  ...
  timestamp:     { type: Date,   required: true },        // fundingTime (8h settlement)
  funding_rate:  { type: Number, required: true },        // actual settled funding rate
  annualized_rate: { type: Number, required: true },      // funding_rate * 3 * 365 * 100
}, { collection: 'binance_funding_8h', timestamps: false });
